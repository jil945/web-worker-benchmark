...
var bigInt = function (undefined) {
	...
	function Integer(v, radix) {
		...
		if (typeof radix !== "undefined") return Number(radix) === 10 ? parseValue(v) : parseBase(v, radix);
		return parseValue(v);
	}
	...
	function shift_isSmall(n) {
		return (typeof n === "number" || typeof n === "string") && Number(Math.abs(n)) <= BASE || n instanceof BigInteger && n.value.length <= 1;
	}
	...
	var parseBase = function (text, base) {
		...
		for (var i = 0; i < length; i++) {
			...
			if (c === "-") continue;
			if (/[a-z0-9]/.test(c)) {
				if (/[0-9]/.test(c) && Number(c) >= absBase) {
	...
	function toBase(n, base) {
		...
		if (base.equals(-1)) {
			...
			return "1" + new Array(Number(n)).join("01");
		}
		var minusSign = "";
		...
		if (base.equals(1)) {
			if (n.isZero()) return "0";
			return minusSign + new Array(Number(n) + 1).join(1);
		}
		var out = [];
		var left = n,
			divmod;
